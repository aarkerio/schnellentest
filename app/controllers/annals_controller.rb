...
class AnnalsController < ApplicationController
  ...
  before_action :set_annal, only: [:show, :edit, :update, :destroy]
  ...
  # POST /annals
  def create
    new_params = Annal.order_params params, current_user.id
    #return render text: new_params
    logger.debug "####  new_params #################>>>  #{new_params.inspect}"
    @annal = Annal.new new_params
    respond_to do |format|
      if @annal.save
        format.html { redirect_to annals_path, notice: 'Annal was successfully created.' }
