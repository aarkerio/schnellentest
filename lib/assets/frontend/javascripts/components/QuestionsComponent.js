...
class QuestionsComponent extends Component {
  ...
  componentWillMount() {
    if ( ! this.props.OneTestArrayProp.length ) {
      console.log('Ich bin hier!! bei componentWillMount');
  ...
  */
  deleteQuestion(question_id) {
    let action = TestsActionCreators.deleteQuestion(question_id);
    this.props.dispatch(action);
  ...
  render() {
    return (
      ...
      <div>
        <Link to={"/questionnew/"+this.props.routeParams.test_id+"/"}> 
