...
export function searchQuestions(test_id, terms) {
    return function (dispatch) {
      let data = {
        method:      'POST',
        credentials: 'same-origin',
...
function setSearch(QuestionsArrayProp) {
  return {
...
export function addQuestions(test_id, ids) {
  let question_ids = [];

  for (var i = 0; i < ids.length; i++){
     console.log(' id >>>' + i + ' ids[id] >>>>' + ids[i]);
     question_ids.push({question_ids: ids[i]});
  }
