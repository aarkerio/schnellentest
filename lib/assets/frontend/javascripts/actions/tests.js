...
export function fetchOneTest(test_id) {
    console.log('>>>>>>>>>>>>>  test_id in setOneTest: ' + test_id);
    return function (dispatch) {
      let data = {
        method:      'POST',
        credentials: 'same-origin',
...
export function deleteTest(id){
     return function (dispatch) {
      ...
      return fetch('/api/v1/tests/delete/'+id, data)
          .then(response => response.json())
...
function setOneQuestion(OneQuestionArrayProp) {
  console.log('>>>>>>>>>>>>>   setOneQuestion: ' + JSON.stringify(OneQuestionArrayProp));
  return {
...
export function deleteQuestion(id){
     return function (dispatch) {
      ...
      return fetch('/api/v1/question/delete/'+id, data)
          .then(response => response.json())
          .then(json => console.log('Deleted id:  ' + id));
  };
};
